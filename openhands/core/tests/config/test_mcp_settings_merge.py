...
def test_mcp_config_merge_stdio_servers():
    ...
    config2 = MCPConfig(
        stdio_servers=[
            MCPStdioServerConfig(name='server3', command='java', args=['-jar', 'app.jar']),
            MCPStdioServerConfig(name='server4', command='go', env={'GO_ENV': 'production'}),
        ]
    )
...
def test_mcp_config_merge_complex_scenario():
    """Test merging with a complex scenario involving different server configurations."""
    config1 = MCPConfig(
        sse_servers=[
